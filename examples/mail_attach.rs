...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    let acs_client = ACSClientBuilder::new()
        ...
        .build().expect("Failed to build ACS client");

    let mail_attach = EmailAttachmentBuilder::new().file_to_base64("test_file.txt").build().expect("Failed to build EmailAttachment");
